...
parser.add_argument('--name', type=str, help='file name', default='1')
parser.add_argument('--epochs', type=int, help='number of epochs', default=5000)
parser.add_argument('--hidden_size', type=int, help='hidden size of the LSTM', default=256)
...
def load_data(data_path, num_columns=12):
    ...
    with open(data_path, 'r') as file:
        ...
        rows = list(reader)
        for row in tqdm(rows[1:], desc="Loading data"):  # Skip the header row
...
def create_sequences(grouped_data, last_known_state, input_seq_len=10, output_seq_len=30, is_test_data=False):
    ...
    for (case_id, track_id), states in tqdm(grouped_data.items(), desc="Creating sequences"):
        # Sort the states by timestamp_ms
        states.sort(key=lambda x: x[0])
        # Create sequences
        for i in range(len(states) - input_seq_len - output_seq_len + 1):
            # Check if the timestamps are continuous
            ...
            if all(t2 - t1 == 100 for t1, t2 in zip(timestamps, timestamps[1:])):
                ...
                # Ensure the sequences are of correct length
                if len(input_seq) == input_seq_len and len(output_seq) == output_seq_len:
                    input_data.append(input_seq)
                    output_data.append(output_seq)
    if not input_data or (not is_test_data and not output_data):  # Check if input_data and output_data are empty
...
def normalize_data(input_data, output_data, input_scaler=None, output_scaler=None):
    ...
    # Flatten the input data for normalization
    input_data_flat = np.array(input_data).reshape(-1, input_shape[1]*input_shape[2])
    
    # Fit the input scaler if not provided
    ...
    if output_data:
        ...
        # Flatten the output data for normalization
...
# Load, group, and create sequences for the training, test, and validation data
...
class LSTM(nn.Module):
    def __init__(self, input_size=8, hidden_size=args.hidden_size, output_size=8):
        super(LSTM, self).__init__()
        self.lstm = nn.LSTM(input_size, hidden_size, batch_first=True)
        self.fc = nn.Linear(hidden_size, output_size)
        self.output_size = output_size

    def forward(self, x, seq_len, hidden_cell=None):
        outputs = torch.zeros((x.size(0), seq_len, self.output_size)).to(x.device)
...
for epoch in range(num_epochs):
    ...
    for input_data, output_data in val_dataloader:
        ...
        output_data = output_data.view(-1, 30, 8)
        loss = criterion(output.view(-1, 8), output_data.view(-1, 8))
...
model.eval()
with torch.no_grad():
    input_data = torch.FloatTensor(test_input_data[target_agent]).unsqueeze(0)
    output, _ = model(input_data, 30)  # Pass the sequence length to the model
...
train_predicted_output, _ = model(torch.FloatTensor(train_input_data), 30)
train_predicted_output = train_predicted_output.detach().numpy()
train_actual_output = np.array(train_output_data)

val_predicted_output, _ = model(torch.FloatTensor(val_input_data), 30)
val_predicted_output = val_predicted_output.detach().numpy()
val_actual_output = np.array(val_output_data)

# Flatten the time steps and features dimensions
...
# Calculate MRE for # Calculate MAE for validation data
...
# Calculate Explained Variance Score for training, and validation data
...
# Calculate ADE for training, and validation data
...
# Calculate minFDE for training, and validation data
train_fde = np.sqrt(np.sum((train_predicted_output[:, -1, :] - train_actual_output[:, -1, :])**2, axis=-1))
train_avgFDE = np.mean(train_fde)
print(f'Training minFDE: {train_avgFDE}')

# Calculate minFDE for validation data
...
# Calculate Miss Rate for training, and validation data
...
def plot_trajectory(input_data, predicted_output, actual_output, title):
    ...
    # Save the plot
    # plt.savefig(os.path.join(os.path.dirname(path), f'LSTM-Trajectory-{title}-{args.name}.png'))
